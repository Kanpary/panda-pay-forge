...
if ($isLoggedIn) {
    try {
$stmt = $pdo->prepare("SELECT saldo, cpf, nome_completo FROM users WHERE id = ?");
...
$user = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        // Error handling, assume not logged in or keep existing session but 0 balance
    }
}

// Fetch initial roulette maps
$map1 = [0, 10, 0, 50, 0, 20, 0, 100]; // Default
$map2 = [1, 2, 3, 4, 1, 2, 3, 4]; // Default

try {
    $stmt = $pdo->query("SELECT valor FROM probabilidade ORDER BY id ASC");
    $fetched1 = $stmt->fetchAll(PDO::FETCH_COLUMN);
    if (!empty($fetched1)) {
        $map1 = $fetched1;
    }

    $stmt = $pdo->query("SELECT valor FROM multiplicadores ORDER BY id ASC");
    $fetched2 = $stmt->fetchAll(PDO::FETCH_COLUMN);
    if (!empty($fetched2)) {
        $map2 = $fetched2;
    }
...
$minDeposit = isset($settings['min_deposit']) ? floatval($settings['min_deposit']) : 10.00;
$minWithdrawal = isset($settings['min_withdrawal']) ? floatval($settings['min_withdrawal']) : 50.00;
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Roleta Natalina</title>
    <meta name="description" content="">
    <meta name="keywords" content="">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Roleta Natalina">
    <meta property="og:description" content="">
    <meta property="og:image" content="">

    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:title" content="Roleta Natalina">
...
<body>
    <!-- Neve Caindo no Site Inteiro -->
    <div id="global-snow-container">
    ...
    <audio id="background-music" loop autoplay>
        <source src="/assets/sounds/natal.mp3" type="audio/mpeg">
    </audio>
    ...
    <div id="game-container">
        ...
        <img id="bonus-wheel" class="swaying" src="assets/img/roleta11.png" alt="Roleta bônus">
        <div id="roleta2-labels" class="labels-container swaying"></div>
        <img id="bonus-indicator" src="assets/img/ceta.png" alt="Indicador de bônus">
        ...
        <button id="normal-button" class="ui-button">⚡ NORMAL</button>
        ...
        <img id="play-button" src="assets/img/<?php echo $isLoggedIn ? 'girar10.png' : 'cadastro11.png'; ?>" alt="Botão de jogar/cadastrar">

        <div id="giro-area">
            <button id="plus-button" class="ui-button" <?php echo !$isLoggedIn ? 'disabled' : ''; ?>>+</button>
            <div id="giro-block" class="ui-panel">
                <p class="label">Giro</p>
                <p id="giro-value" class="value">R$ 0,50</p>
            </div>
            <button id="minus-button" class="ui-button" <?php echo !$isLoggedIn ? 'disabled' : ''; ?>>-</button>
        </div>

        <div id="modal-ganhos-overlay" class="modal-overlay">
            <div id="modal-content">
                <h2 id="modal-title">Parabéns!</h2>
                <p id="modal-ganho">Você ganhou R$ 50,00</p>
        ...
        <div id="profile-modal-overlay" class="modal-overlay">
            <div id="profile-modal-content">
                ...
                <button id="open-history-button" class="profile-button">Histórico</button>
                <button id="open-affiliate-button" class="profile-button">Afiliado</button>
                <button id="open-affiliate-history-button" class="profile-button">Histórico de Afiliado</button>
        ...
        <div id="auth-modal-overlay" class="modal-overlay">
            <div class="auth-modal-content">
                <span class="christmas-decoration top-left">🎄</span>
                <span class="christmas-decoration top-right">⭐</span>
                <span class="christmas-decoration bottom-left">❄</span>
                <span class="christmas-decoration bottom-right">🎁</span>
    ...
    <p class="form-helper">Formato: 000.000.000-00</p>
</div>
        ...
        $('#cpf').mask('000.000.000-00');
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-wrapper">
                                ...
                                <input type="password" name="senha" class="form-input" placeholder="Crie uma senha (mín. 6 caracteres)" required minlength="6">
                        ...
                        <p class="form-footer">Já tem uma conta? <a href="#" class="footer-link" id="switch-to-login">Entrar</a></p>
                    </form>
                </div>
    ...
    <div id="help-modal-overlay">
        <div id="help-modal-content">
            ...
            <div class="help-tabs">
                ...
                <button class="help-tab" data-tab="deposito">Depósito</button>
            ...
            <div class="help-content">
                ...
                <div class="help-section active" id="help-jogar">
                    ...
                    <p>Bem-vindo ao nosso jogo de roleta! Teste sua sorte e ganhe prêmios incríveis.</p>
                    
                    <h4>🎮 Passo a Passo</h4>
                    <ul>
                        <li><strong>1. Escolha sua aposta:</strong> Use os botões <strong>+</strong> e <strong>-</strong> para definir o valor do giro</li>
                        <li><strong>2. Clique em START:</strong> Pressione o botão central para girar as roletas</li>
                        <li><strong>3. Aguarde o resultado:</strong> As roletas vão girar e parar em posições aleatórias</li>
                        <li><strong>4. Receba seu prêmio:</strong> Se ganhar, o valor será creditado automaticamente no seu saldo</li>
                    </ul>

                    <h4>🎰 Sistema de Roletas</h4>
                    <p><strong>Roleta Principal (Externa):</strong> Define o valor base do seu prêmio. Pode variar de 0x até 100x sua aposta!</p>
                    <p><strong>Roleta Bônus (Interna):</strong> Multiplica o valor da roleta principal por 1x, 2x, 3x ou 4x!</p>

                    <h4>💎 Cálculo de Prêmios</h4>
                    <p><strong>Prêmio Final =</strong></p>
                    <p>Aposta × Roleta Principal × Multiplicador Bônus</p>
                    <p><strong>Exemplo:</strong> R$ 1,00 × 50x × 3x = R$ 150,00</p>

                    <h4>💡 Dicas Importantes</h4>
                    <ul>
                        <li>Quanto maior a aposta, maiores os prêmios potenciais</li>
                        <li>Gerencie seu saldo com responsabilidade</li>
                        <li>Não há padrões ou horários "quentes" - cada giro é aleatório</li>
                        <li>Defina limites e jogue por diversão</li>
                    </ul>

                    <h4>⚡ Modo de Jogo</h4>
                    <p><strong>Modo Flash:</strong> Giros rápidos de 3 segundos para mais emoção e agilidade!</p>
                </div>

                <!-- Seção Depósito -->
                <div class="help-section" id="help-deposito">
                    <h3>💳 Como Fazer um Depósito</h3>
                    <p>Adicione saldo à sua conta de forma rápida e segura. Siga os passos abaixo:</p>
                    
                    <h4>📝 Passo a Passo</h4>
                    <ul>
                        <li><strong>1. Clique no botão "Depositar":</strong> Localizado no canto superior direito</li>
                        <li><strong>2. Escolha o valor:</strong> Selecione ou digite o valor que deseja depositar</li>
                        <li><strong>3. Método de pagamento:</strong> Escolha entre PIX, cartão de crédito ou outros métodos disponíveis</li>
                        <li><strong>4. Confirme os dados:</strong> Revise o valor e método escolhido</li>
                        <li><strong>5. Realize o pagamento:</strong> Siga as instruções na tela (QR Code para PIX, dados do cartão, etc.)</li>
                        <li><strong>6. Aguarde a confirmação:</strong> O saldo é creditado automaticamente após aprovação</li>
                    </ul>

                    <h4>⚡ Métodos de Pagamento</h4>
                    <p><strong>💰 PIX</strong> - Instantâneo • Disponível 24/7 • Sem taxas</p>
                    <p><strong>💳 Cartão de Crédito</strong> - Aprovação imediata • Parcele em até 3x</p>
                    <p><strong>🏦 Transferência</strong> - Depósitos maiores • Confirmação em até 2h úteis</p>

                    <h4>⏱️ Tempo de Processamento</h4>
                    <ul>
                        <li><strong>PIX:</strong> Imediato (até 5 minutos)</li>
                    </ul>

                    <h4>🔒 Segurança</h4>
                    <p>Todos os pagamentos são processados através de gateways certificados e criptografados. Seus dados financeiros estão 100% protegidos.</p>

                    <h4>💡 Dicas</h4>
                    <ul>
                        <li>Guarde o comprovante até a confirmação do depósito</li>
                        <li>Valores mínimos podem variar conforme o método</li>
                        <li>Em caso de dúvidas, entre em contato com o suporte</li>
                        <li>Não compartilhe códigos PIX com terceiros</li>
                    </ul>
                </div>

                <!-- Seção Saque -->
                <div class="help-section" id="help-saque">
                    <h3>💰 Como Sacar seus Ganhos</h3>
                    <p>Resgate seus ganhos de forma simples e segura. Confira o passo a passo:</p>
                    
                    <h4>📝 Passo a Passo</h4>
                    <ul>
                        <li><strong>1. Acesse seu perfil:</strong> Clique no ícone de perfil no canto superior direito</li>
                        <li><strong>2. Clique em "Sacar":</strong> Ou use o botão de saque no menu</li>
                        <li><strong>3. Informe o valor:</strong> Digite quanto deseja sacar (respeitando o mínimo)</li>
                        <li><strong>4. Escolha o método:</strong> Selecione PIX, transferência ou outro método disponível</li>
                        <li><strong>5. Informe os dados:</strong> CPF, chave PIX ou dados bancários</li>
                        <li><strong>6. Confirme a solicitação:</strong> Revise todos os dados antes de confirmar</li>
                        <li><strong>7. Aguarde o processamento:</strong> O prazo varia conforme o método escolhido</li>
                    </ul>

                    <h4>⚙️ Requisitos para Saque</h4>
                    <ul>
                        <li><strong>Valor mínimo:</strong> R$ 20,00</li>
                    ...
                    <h4>⏱️ Tempo de Processamento</h4>
                    <ul>
                        <li><strong>PIX:</strong> Até 30 minutos (em horário comercial)</li>
                        <li><strong>Transferência Bancária:</strong> 1 a 2 dias úteis</li>
                        <li><strong>Finais de semana:</strong> Processamento na segunda-feira</li>
                    </ul>

                    <h4>📋 Políticas de Saque</h4>
                    <ul>
                        <li>Solicitações são processadas de segunda a sexta, 9h às 18h</li>
                        <li>Você pode ter até 3 solicitações pendentes simultaneamente</li>
                        <li>Verificação de identidade pode ser solicitada</li>
                        <li>Taxa de saque: Consulte a tabela no momento da solicitação</li>
                    </ul>

                    <h4>⚠️ Atenção</h4>
                    <p><strong>Importante:</strong> Certifique-se de que seus dados bancários estão corretos. Transferências para contas de terceiros não são permitidas por questões de segurança.</p>

                    <h4>💡 Dicas</h4>
                    <ul>
                        <li>Cadastre sua chave PIX com antecedência</li>
                        <li>Verifique o status do saque no histórico</li>
                        <li>Guarde o comprovante de cada transação</li>
                ...
                <div class="help-section" id="help-afiliados">
                    ...
                    <p>Ganhe comissões indicando amigos! Quanto mais pessoas jogarem através do seu link, mais você ganha.</p>
                    
                    <h4>🚀 Como Funciona</h4>
                    <ul>
                        <li><strong>1. Acesse o menu Afiliados:</strong> No seu perfil, clique em "Afiliados"</li>
                        <li><strong>2. Copie seu link:</strong> Cada usuário tem um link exclusivo de indicação</li>
                        <li><strong>3. Compartilhe:</strong> Envie para amigos, redes sociais, WhatsApp, etc.</li>
                        <li><strong>4. Ganhe comissões:</strong> Receba uma porcentagem dos depósitos e jogadas</li>
                    </ul>

                    <h4>💰 Tipos de Comissão</h4>
                    <h4>🎯 CPA (Custo por Aquisição)</h4>
                    <p>Ganhe um <strong>valor fixo</strong> por cada novo usuário que se cadastrar através do seu link e realizar o primeiro depósito.</p>
                    <p><strong>Exemplo:</strong> R$ 10,00 por cada indicado que depositar</p>

                    <h4>📊 RevShare (Revenue Share)</h4>
                    <p>Ganhe uma <strong>porcentagem recorrente</strong> sobre os depósitos dos seus indicados enquanto eles jogarem.</p>
                    <p><strong>Exemplo:</strong> 5% de cada depósito realizado pelo indicado</p>

                    <h4>📈 Níveis de Afiliado</h4>
                    <p><strong>🥉 Iniciante</strong> - 0-10 indicados ativos - Comissão: 3% RevShare</p>
                    <p><strong>🥈 Intermediário</strong> - 11-50 indicados ativos - Comissão: 5% RevShare</p>
                    <p><strong>🥇 Avançado</strong> - 51-100 indicados ativos - Comissão: 7% RevShare</p>
                    <p><strong>💎 Master</strong> - 100+ indicados ativos - Comissão: 10% RevShare</p>

                    <h4>📊 Acompanhamento</h4>
                    <p>No painel de afiliados você pode visualizar:</p>
                    <ul>
                        <li><strong>Total de indicados:</strong> Quantas pessoas usaram seu link</li>
                        <li><strong>Indicados ativos:</strong> Quantos realizaram depósito</li>
                        <li><strong>Ganhos totais:</strong> Quanto você já ganhou com comissões</li>
                        <li><strong>Histórico detalhado:</strong> Veja cada transação que gerou comissão</li>
                    </ul>

                    <h4>💳 Recebendo suas Comissões</h4>
                    <ul>
                        <li>As comissões são creditadas automaticamente no seu saldo</li>
                        <li>Você pode sacar suas comissões a qualquer momento</li>
                        <li>Não há valor mínimo diferenciado para saque de comissões</li>
                        <li>Histórico completo disponível na aba Afiliados</li>
                    </ul>

                    <h4>💡 Dicas para Sucesso</h4>
                    <ul>
                        ...
                        <li>Crie conteúdo sobre o jogo (vídeos, tutoriais, etc.)</li>
                        <li>Quanto mais indicados ativos, maior sua comissão!</li>
                    </ul>

                    <h4>⚠️ Regras do Programa</h4>
                    <ul>
                        <li>Proibido criar múltiplas contas para autoindicação</li>
                        <li>Não utilize métodos de spam ou práticas abusivas</li>
                        <li>Comissões de atividades fraudulentas serão canceladas</li>
                        <li>A plataforma se reserva o direito de suspender afiliados que violarem as regras</li>
                    </ul>

                    <h4>🎁 Bônus Especiais</h4>
                    <p>Fique atento a campanhas especiais onde você pode ganhar bônus extras por metas atingidas, como comissões dobradas em determinados períodos!</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modal-card deposit-card">
            ...
            <div class="modal-body">
                <form id="depositForm" class="deposit-form">
                    <h2 class="deposit-title">Depositar</h2>
                    <div class="input-group">
                        ...
                        <small class="min-value-notice">O valor mínimo é R$ <?php echo number_format($minDeposit, 2, ',', '.'); ?></small>
                    </div>
                    ...
                    <button type="submit" class="generate-btn"><i class="bi bi-qr-code"></i> Gerar QR Code PIX</button>
                </form>
                <div id="qrArea" class="qr-section">
                    ...
                    <div class="qr-image-wrapper">
                        ...
                        <div id="qrPaidOverlay" class="qr-paid-overlay">PAGO ✅</div>
                    </div>
...
<section id="withdrawModal" class="modal-container">
    ...
    <div class="input-group compact">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <label class="input-label" style="margin-bottom: 0;">Valor do saque</label>
            <small style="color: #9ca3af; font-size: 12px;">
                Mínimo: R$ <?php echo number_format($minWithdrawal, 2, ',', '.'); ?>
            </small>
        </div>
        <input type="text" name="amount" id="withdrawAmount" required
               class="amount-input" placeholder="R$ 0,00" inputmode="numeric">
        <small class="max-balance">Seu Saldo: R$ 0,00</small>
    </div>

    <div id="withdrawRolloverNotice" class="rollover-notice">
        Você ainda precisa apostar <strong id="withdrawRolloverAmount">R$ 0,00</strong>.
    </div>

    <!-- CHAVE PIX FIXA COMO CPF -->
    <div class="input-group compact">
        <label class="input-label">Chave PIX</label>
        <div class="pix-input-container">

            <!-- Select travado apenas CPF -->
            <select name="pixKeyType" id="pixKeyType"
                    class="pix-selector compact-select" disabled>
            ...
            <input type="text"
                   name="pixKey"
                   id="pixKey"
                   class="pix-input compact-input"
                   value="<?php echo htmlspecialchars($cpf); ?>"
                   readonly>
        </div>
    </div>

    <!-- NOME E CPF (SOMENTE LEITURA) -->
    ...
    <button type="submit" class="generate-btn compact-submit">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const DEPOSITO_MIN = <?php echo $minDeposit; ?>;
        const BONUS_PERCENT = 10;
        const depositValueButtons = [20, 30, 50, 100];
        const grid = document.querySelector('#depositModal .values-grid');
        let paymentCheckInterval = null;

        if (grid) {
            grid.innerHTML = depositValueButtons.map((val, index) => {
                const isQuente = val === 30;
                const quenteTag = isQuente ? '<span class="quente-tag">🔥 QUENTE</span>' : '';
                return `<button type="button" data-value="${val}" class="value-btn ${isQuente ? 'quente' : ''}">${quenteTag}R$ ${val.toFixed(2).replace('.', ',')}</button>`;
            }).join('');
        }
                ...
                try {
                    ...
                    if (data.status === 'PAID') {
                        ...
                        if (typeof Notiflix !== 'undefined') {
                            Notiflix.Notify.success('Depósito confirmado com sucesso!');
                        }
                        setTimeout(() => {
        ...
        document.getElementById('depositForm')?.addEventListener('submit', async function(e) {
            ...
            if (isNaN(value) || value < DEPOSITO_MIN) {
                if (typeof Notiflix !== 'undefined') {
                    Notiflix.Notify.failure(`O valor mínimo para depósito é R$ ${DEPOSITO_MIN.toFixed(2).replace('.', ',')}`);
                }
                return;
            }
            const form = e.target;
            ...
            try {
                ...
                if (data.qrcode && data.transactionId) {
                    const qrArea = document.getElementById('qrArea');
                    document.getElementById('qrImg').src = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(data.qrcode)}`;
                    document.getElementById('qrCodeValue').value = data.qrcode;
                    qrArea.classList.add('active');
                    startPaymentPolling(data.transactionId);
                } else {
                    if (typeof Notiflix !== 'undefined') {
                        Notiflix.Notify.failure(data.error || 'Erro ao gerar QR Code.');
                    }
        ...
        document.getElementById('withdrawForm')?.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (typeof Notiflix !== 'undefined') Notiflix.Notify.failure(error.message || 'Erro de conexão.');
            } finally {
                if (typeof Notiflix !== 'undefined') Notiflix.Loading.remove();
            }
        });
        ...
        document.getElementById('copyQr')?.addEventListener('click', function() {
            ...
            navigator.clipboard.writeText(qrCodeInput.value).then(() => {
                ...
                if (typeof Notiflix !== 'undefined') {
                    Notiflix.Notify.success('Código PIX copiado com sucesso!');
                }
                setTimeout(() => {
                ...
                if (typeof Notiflix !== 'undefined') {
                    Notiflix.Notify.failure('Não foi possível copiar o código. Tente manualmente.');
                }
            });
        });
    });
...
<div id="history-modal-overlay" class="modal-overlay">
    <div class="history-modal-content">
        <div class="history-modal-header">
            <h2 class="history-modal-title">Histórico de Jogos</h2>
        ...
        <div id="affiliate-modal-body">
            ...
            <div class="affiliate-section">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                    <h3 class="section-title" style="margin: 0;"><i data-lucide="bar-chart-2"></i> Suas Estatísticas</h3>
                    <div style="display: flex; align-items: center; gap: 5px; font-size: 0.75rem; color: #aaa;">
                        ...
                        <span>Comissão: <strong id="comissao-percentual" style="color: #48bb78; font-size: 0.85rem;">0%</strong></span>
                    </div>
                </div>
...
<!-- Modal de Histórico Completo de Depósitos -->
<div id="depositos-completo-modal-overlay" class="modal-overlay" style="display: none;">
    <div class="affiliate-modal-content" style="max-width: 700px; max-height: 90vh;">
        <div class="affiliate-modal-header">
            <h2 class="affiliate-modal-title"><i data-lucide="list"></i> Histórico de Afiliado</h2>
        ...
        <div class="affiliate-section" style="margin-bottom: 15px;">
            <div class="stats-grid-affiliate" style="grid-template-columns: repeat(3, 1fr);">
                <div class="stat-item-affiliate">
                    ...
                    <p class="label">Total de Depósitos</p>
                </div>
                <div class="stat-item-affiliate">
...
.deposito-completo-item .valor-ganho {
    ...
    <script>
    // Verifica imediatamente se há um registro recente (antes do DOMContentLoaded)
    const shouldOpenDeposit = sessionStorage.getItem('justRegistered') === 'true' && false;
    
    document.addEventListener('DOMContentLoaded', () => {
        // Affiliate Logic: Capture and store referral code from URL
        ...
        // Apply stored referral code to registration form
        const storedRefCode = sessionStorage.getItem('affiliate_code');
        if (storedRefCode) {
            const refInput = document.querySelector('input[name="ref"]');
            if (refInput) {
                refInput.value = storedRefCode;
            }
        }
        ...
        const state = {
            ...
            currentRotation1: 0,
            currentRotation2: 0,
            roleta1Map: <?php echo json_encode($map1); ?>,
            roleta2Map: <?php echo json_encode($map2); ?>
        };
        ...
        if (state.isLoggedIn) {
            setInterval(() => {
                fetch('api/get_saldo.php')
                    ...
                    .then(data => {
                        if (data.success) {
                            state.saldo = parseFloat(data.saldo);
                            const saldoElement = document.getElementById('saldo-value');
                            if (saldoElement) {
                                saldoElement.textContent = `R$ ${state.saldo.toFixed(2).replace('.', ',')}`;
                            }
                            
                            const withdrawBalanceEl = document.querySelector('#withdrawModal .max-balance');
                            if (withdrawBalanceEl) {
                                withdrawBalanceEl.textContent = `Seu Saldo: R$ ${state.saldo.toFixed(2).replace('.', ',')}`;
                            }
                        }
                    })
        ...
        if (backgroundMusic) {
            backgroundMusic.volume = 0.5; // Volume padrão (50%)
            ...
            // Se o autoplay foi bloqueado, tocar na primeira interação do usuário
        ...
        const updateDisplay = () => {
            ...
            // Adiciona animação se o valor mudou
        ...
        const showGanhosModal = (ganho, m1, m2) => {
            if (ganho > 0) {
                DOMElements.modalTitle.textContent = 'Parabéns!';
                DOMElements.modalGanho.textContent = `Você ganhou ${formatCurrency(ganho)}`;
                DOMElements.modalDetails.textContent = `(${m1}x) multiplicado por (${m2}x)`;
            } else {
                DOMElements.modalTitle.textContent = 'Não foi dessa vez!';
                DOMElements.modalGanho.textContent = 'Você não ganhou nada.';
        ...
        const hideGanhosModalAndResume = async () => {
            ...
            try {
                const response = await fetch('api/finish_spin.php', { method: 'POST' });
                const result = await response.json();
                ...
                showNotification('Erro de conexão ao finalizar jogada.', true);
            }
            toggleSpinButtons(true);
            DOMElements.roleta1.classList.add('swaying'); DOMElements.roleta1Labels.classList.add('swaying');
            DOMElements.roleta2.classList.add('swaying'); DOMElements.roleta2Labels.classList.add('swaying');
        };

        const startSpinAnimation = (winningPos1, winningPos2, multiplicador1Exato = null, multiplicador2Exato = null, finalBalance = null) => {
            ...
            // Usa os multiplicadores exatos da resposta se disponíveis, senão usa do mapa
            ...
            // Calcular o ângulo para posicionar o segmento ganhador no topo (indicador)
            // O indicador está no topo (0 graus), então precisamos rotacionar para que o segmento ganhador fique lá
            const targetAngle1 = 360 - (winningPos1 * segmentAngle1) - (segmentAngle1 / 2);
            // Adicionar rotações completas para efeito visual e garantir que pare no lugar certo
            ...
            // Mesma lógica para a roleta 2
            const targetAngle2 = 360 - (winningPos2 * segmentAngle2) - (segmentAngle2 / 2);
            // Inverte o sinal de targetAngle2 para somar, assim como na roleta 1, para alinhar corretamente
            ...
            setTimeout(() => {
                // Atualiza o saldo com o valor final recebido do backend
                if (finalBalance !== null) {
                    state.saldo = finalBalance;
                    updateDisplay();
                }
        ...
        const handlePlay = async () => {
            ...
            if (state.saldo < state.aposta) {
                DOMElements.saldoValue.style.color = 'red';
                DOMElements.saldoValue.classList.add('updated');
                setTimeout(() => { 
            ...
            try {
                const response = await fetch('api/start_spin.php', { method: 'POST', body: formData });
                const result = await response.json();
                ...
                if (!result.success) {
                    ...
                    if (result.message === 'Saldo insuficiente.') {
                        const saldoRes = await fetch('api/get_saldo.php');
                ...
                // Deduz visualmente a aposta imediatamente para melhor UX
                // O saldo final será atualizado ao fim da animação
                state.saldo = Math.max(0, state.saldo - state.aposta);
                ...
                updateDisplay();
                // Passa também os multiplicadores exatos da resposta e o saldo final
                startSpinAnimation(result.posicao1, result.posicao2, result.multiplicador1, result.multiplicador2, result.newBalance);
            } catch (error) {
                showNotification('Erro de conexão. Tente novamente.', true);
                toggleSpinButtons(true);
            }
        };
        ...
        const handleAuthFormSubmit = async (e, action) => {
            ...
            try {
                ...
                if (result.success) {
                    if (action === 'register') {
                        // Se deve abrir depósito, marca flag
        ...
        const openHistoryModal = async () => {
            ...
            DOMElements.historyList.innerHTML = '<p id="loading-history">Carregando histórico...</p>';
            try {
                ...
                if (result.success && result.history.length > 0) {
                    ...
                    DOMElements.historyList.innerHTML = '<p id="no-history-message">Nenhum jogo encontrado no seu histórico.</p>';
                }
            } catch (error) {
                DOMElements.historyList.innerHTML = '<p id="no-history-message" class="result-loss">Erro ao carregar o histórico.</p>';
            }
        };
        ...
        const openAffiliateModal = async () => {
            ...
            try {
                const response = await fetch('api/get_affiliate_data.php');
                const data = await response.json();
                if (!data.success) throw new Error(data.message);

                // Salvar dados globalmente para uso no modal de histórico
                ...
                // Mostrar porcentagem de comissão
                const comissaoPercentualEl = document.getElementById('comissao-percentual');
                if (comissaoPercentualEl) {
                    if (data.commission_type === 'cpa') {
                        const cpaVal = parseFloat(data.cpa_value || 0);
                        comissaoPercentualEl.textContent = 'CPA ' + cpaVal.toLocaleString('pt-BR', {
                            style: 'currency',
                            currency: 'BRL'
                        });
                    } else {
        ...
        const initEventListeners = () => {
            ...
            DOMElements.plusButton.addEventListener('click', () => { if (!state.isSpinning) { state.aposta += state.apostaStep; updateDisplay(); } });
            DOMElements.minusButton.addEventListener('click', () => { if (!state.isSpinning && state.aposta > state.apostaStep) { state.aposta -= state.apostaStep; updateDisplay(); } });
            DOMElements.normalButton.addEventListener('click', () => { if (!state.isSpinning) { state.isNormal = !state.isNormal; DOMElements.normalButton.innerHTML = state.isNormal ? '⚡ NORMAL' : '⚡ FLASH'; } });
            
            // Botão de depósito
            if (DOMElements.depositButton) {
                DOMElements.depositButton.addEventListener('click', () => {
                    if (state.isLoggedIn) {
                        // Se estiver logado, abre o modal de depósito
                        ...
                        // Se não estiver logado, abre o modal de cadastro
                        showAuthModal('register');
                    }
                });
            }
            ...
            // Link "Entrar" no rodapé do formulário de cadastro
            ...
            if (DOMElements.openWithdrawButton) {
                DOMElements.openWithdrawButton.addEventListener('click', () => {
                    if (typeof openWithdrawModal === 'function') {
                        ...
                        alert('Erro: Modal de saque não encontrado.');
                    }
                });
            }
            ...
            if (DOMElements.openDepositButton) {
                DOMElements.openDepositButton.addEventListener('click', () => {
                    if (typeof openDepositModal === 'function') {
                        ...
                        alert('Erro: Modal de depósito não encontrado.');
                    }
                });
            }
            ...
            if (DOMElements.historyModalOverlay) DOMElements.historyModalOverlay.addEventListener('click', e => { if (e.target === DOMElements.historyModalOverlay) DOMElements.historyModalOverlay.classList.remove('show'); });
            ...
            if (DOMElements.affiliateModalOverlay) DOMElements.affiliateModalOverlay.addEventListener('click', e => { if (e.target === DOMElements.affiliateModalOverlay) DOMElements.affiliateModalOverlay.classList.remove('show'); });
            
            // Botão Ver Depósitos Trazidos (no painel de afiliado)
            ...
            if (verDepositosBtn) {
                verDepositosBtn.addEventListener('click', () => {
                    if (window.affiliateData && window.affiliateData.depositos_completos) {
                        ...
                        // O saldo disponível é igual ao total recebido
                        ...
                        // Se não tiver dados, carregar
                        openAffiliateHistoryModal();
                    }
                });
            }
            
            // Função para abrir modal de histórico de afiliado
            window.openAffiliateHistoryModal = async function() {
                ...
                if (!depositosCompletoModal) {
                    ...
                    showNotification('Erro: Modal não encontrado', true);
                    return;
                }
                ...
                if (!lista) {
                    console.error('Lista de depósitos não encontrada');
                    return;
                }
                ...
                try {
                    ...
                    // Calcular total recebido e saldo disponível
                    ...
                    if (typeof abrirModalDepositosCompletos === 'function') {
                        ...
                        console.error('Função abrirModalDepositosCompletos não encontrada');
                    }
                } catch (error) {
                    if (lista) {
                        lista.innerHTML = '<p style="text-align: center; color: #ef4444; padding: 20px;">Erro ao carregar histórico: ' + error.message + '</p>';
                    }
                    console.error('Erro ao carregar histórico de afiliado:', error);
                    showNotification('Erro ao carregar histórico de afiliado', true);
                }
            };
            
            // Função para abrir modal de depósitos completos
            window.abrirModalDepositosCompletos = function(depositos, totalRecebido = null, saldoDisponivel = null) {
                ...
                if (histSaldoDisponivel) {
                    // Saldo disponível é sempre igual ao total recebido
                    histSaldoDisponivel.textContent = formatCurrency(valorFinal);
                }
                
                if (!depositos || depositos.length === 0) {
                    lista.innerHTML = '<p style="text-align: center; color: #aaa; padding: 20px;">Nenhum depósito encontrado.</p>';
                    return;
                }
                        ...
                        <div class="deposito-completo-item">
                            <div class="header">
                                <div class="indicado-info">
                                    <div class="indicado-nome">${dep.nome_indicado || 'Usuário #' + dep.indicado_id}</div>
                ...
                // Tentar criar ícones se lucide estiver disponível
            ...
            // Botão Histórico de Afiliado (no menu do perfil)
            if (DOMElements.openAffiliateHistoryButton) {
                DOMElements.openAffiliateHistoryButton.addEventListener('click', () => {
                    if (typeof window.openAffiliateHistoryModal === 'function') {
                        ...
                        console.error('Função openAffiliateHistoryModal não encontrada');
                    }
                });
            }
            
            // Event listeners para fechar modal de histórico
            const depositosCompletoModal = document.getElementById('depositos-completo-modal-overlay');
        ...
        // Sistema de Notificações de Saque em Tempo Real
        const withdrawNames = [
            'João Silva', 'Maria Santos', 'Pedro Oliveira', 'Ana Costa', 'Carlos Souza',
            'Juliana Ferreira', 'Rafael Almeida', 'Fernanda Lima', 'Bruno Rodrigues', 'Camila Araújo',
            ...
            'Gustavo Cardoso', 'Jéssica Teixeira', 'Rodrigo Moreira', 'Patrícia Freitas', 'Diego Castro',
            'Vanessa Mendes', 'André Ramos', 'Tatiana Duarte', 'Ricardo Azevedo', 'Priscila Correia',
            'Marcos Pinto', 'Renata Moura', 'Leandro Machado', 'Daniela Rezende', 'Vinícius Farias',
            'Bianca Campos', 'Eduardo Nascimento', 'Natália Lopes', 'Henrique Viana', 'Letícia Barros',
            'Paulo Guimarães', 'Sabrina Melo', 'Igor Cavalcanti', 'Raquel Pires', 'Fábio Xavier',
        ...
        const showWithdrawNotification = () => {
            ...
            notification.innerHTML = `
                <div class="withdraw-notification-header">
                    <div class="withdraw-notification-icon">💰</div>
        ...
        // Inicia as notificações após um delay inicial
        ...
        const startWithdrawNotifications = () => {
            ...
            setTimeout(() => {
                ...
                // Depois mostra uma notificação a cada 5-12 segundos
        ...
        const checkSession = async () => {
            try {
                ...
                if (result.success) {
                    ...
                    state.saldo = parseFloat(result.saldo);
                    updateDisplay();
                    DOMElements.plusButton.disabled = false;
                    DOMElements.minusButton.disabled = false;
                    DOMElements.authModalOverlay.classList.remove('show');
                }
            } catch (error) {
        ...
        const init = async () => {
            ...
            // Inicia as notificações de saque
            startWithdrawNotifications();
            
            // Abre automaticamente o modal de cadastro se o usuário não estiver logado
            if (!state.isLoggedIn) {
            ...
            // Se veio com parâmetro register=1, também abre o modal de cadastro (compatibilidade)
            const urlParams = new URLSearchParams(window.location.search);
            ...
            // Se veio com parâmetro depositar=1, abre o modal de depósito
            ...
            // Verifica se o usuário acabou de se registrar e abre o modal de depósito
            const justRegistered = sessionStorage.getItem('justRegistered');
            // Se já processamos o shouldOpenDeposit acima, não processar novamente
            if (state.isLoggedIn && justRegistered === 'true' && !shouldOpenDeposit) {
                console.log('Usuário acabou de se registrar, abrindo modal de depósito...', {
                    justRegistered,
                    shouldOpenDeposit,
                    isLoggedIn: state.isLoggedIn
                });
                sessionStorage.removeItem('justRegistered');
                // Função para abrir o modal de depósito diretamente
                ...
                // Aguarda a função openDepositModal estar disponível ou tenta abrir diretamente
                let attempts = 0;
                const maxAttempts = 50; // Máximo de 5 segundos (50 * 100ms)
                const checkAndOpenDeposit = () => {
                    attempts++;
                    // Tenta usar a função se estiver disponível
                    if (typeof window.openDepositModal === 'function') {
                        console.log('Abrindo modal de depósito via função openDepositModal');
                        window.openDepositModal();
                    } 
                    // Se não, tenta abrir diretamente via DOM
                    else if (openDepositDirectly()) {
                        console.log('Modal de depósito aberto diretamente via DOM');
                        // Modal aberto com sucesso
                    } 
                    // Se ainda não funcionou, tenta novamente
                    else if (attempts < maxAttempts) {
                        if (attempts % 10 === 0) {
                            console.log(`Tentativa ${attempts} de abrir modal de depósito...`);
                        }
                        setTimeout(checkAndOpenDeposit, 100);
                    } else {
                        console.warn('Não foi possível abrir o modal de depósito após várias tentativas');
