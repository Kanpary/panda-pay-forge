...
(function() {
    ...
    const isFreeGame = (typeof window !== 'undefined' && (window.location.pathname || '').replace(/\/$/, '').endsWith('freegame'));
    /** true na rota /login: não chamar getBalance/getPandaConfig (evita "Não autenticado") */
    const isLoginPage = (typeof window !== 'undefined' && (window.location.pathname || '').replace(/\/$/, '').endsWith('login'));
    ...
    var NOMES_MODO_ALEATORIOS = ['Clássico', 'Aventura', 'Explorador', 'Lucky', 'Estrela', 'Bambu', 'Panda', 'Ouro', 'Sorte', 'Jóia', 'Top', 'Master', 'Plus', 'Vip'];
    ...
    function calculateCurrentDifficulty() {
        const inc = (typeof window.pandaDifficultyIncrement === 'number') ? window.pandaDifficultyIncrement : difficultyIncrement;
    ...
    function applyPandaConfigData(data) {
        ...
        if (typeof data.withdraw_multiplier === 'number') window.pandaWithdrawMultiplier = data.withdraw_multiplier;
        ...
        window.pandaDifficultyIncrement = (typeof data.difficulty_increment === 'number') ? data.difficulty_increment : 2.0;
    ...
    function startContinuousPandaAnimation() {
        ...
        pandaAnimationInterval = setInterval(() => {
            ...
            const src = (typeof window.__pandaAssetUrl === 'function') ? window.__pandaAssetUrl(frame + '.png') : (frame + '.png');
            panda.src = src;
            pandaAnimationFrame++;
    ...
    function spawnBranch() {
        ...
        var chanceFlower = Math.min(0.90, cfg.chanceFlower || 0);
        var chanceLeaf = Math.min(0.90, cfg.chanceLeaf || 0);
    ...
    document.querySelectorAll('.branch').forEach(branch => {
        ...
        const sameSide = (panda.classList.contains('left') && branch.classList.contains('left')) ||
        ...
        const overlap = (branchRect.top + margemTolerancia) < (pandaRect.bottom - margemTolerancia) && 
    ...
    document.querySelectorAll('.flower, .leaf').forEach(el => {
        ...
        const overlapping = elRect.top < pandaRect.bottom && elRect.bottom > pandaRect.top &&
                            elRect.left < pandaRect.right && elRect.right > pandaRect.left;
    ...
    async function verificarMetaAtingida() {
        ...
        const multV = (typeof window.pandaWithdrawMultiplier === 'number' ? window.pandaWithdrawMultiplier : 10);
        ...
        if (typeof pandaResult === 'function') {
            try {
                const res = await pandaResult(progresso, valorApostaRodada, { game_session_id: currentGameSessionId });
    ...
    async function startCountdown() {
        ...
        if (!isFreeGame && valorApostaEl) {
            ...
            const saldoAtual = (typeof balanceFromApi === 'number' ? balanceFromApi : (window.balanceFromApi != null ? Number(window.balanceFromApi) : 0)) || 0;
    ...
    function startGame() {
        ...
        difficultyIncrement = (typeof window.pandaDifficultyIncrement === 'number') ? window.pandaDifficultyIncrement : 2;
        obstaclesPassedCount = 0;
        timer = 0;
        metaAtingidaEstaRodada = false;
        valorCreditadoMetaEstaRodada = 0;
        const multVal = (typeof window.pandaWithdrawMultiplier === 'number' ? window.pandaWithdrawMultiplier : 10);
        if (!isFreeGame && valorApostaRodada > 0) {
    ...
    var giftChance = Math.max(0.20, 0.90 - (calculateCurrentDifficulty() / 100 * 0.65));
    ...
    caixa.onclick = () => {
        ...
        if (caixaSecondAt > 0) window.__pandaCaixaTimeouts.push(setTimeout(showCaixaMagica, caixaSecondAt));
    ...
    async function endGame(perdeu = false) {
        ...
        if (typeof window.__pandaCaixaTimeouts !== 'undefined' && Array.isArray(window.__pandaCaixaTimeouts)) {
            window.__pandaCaixaTimeouts.forEach(function(id) {
                clearTimeout(id);
            });
            window.__pandaCaixaTimeouts = [];
        ...
        if (modoHardcore || perdeu) {
            ...
            const perdeuAposMeta = !isFreeGame && perdeu && metaAtingidaEstaRodada && valorApostaPerdida > 0;
            const valorMetaPerdida = perdeuAposMeta ? (valorCreditadoMetaEstaRodada > 0 ? valorCreditadoMetaEstaRodada : valorApostaPerdida * (typeof window.pandaWithdrawMultiplier === 'number' ? window.pandaWithdrawMultiplier : 10)) : 0;
  ...
  `;
        ...
        document.getElementById('voltarSaqueValor').onclick = (typeof onVoltar === 'function' ? onVoltar : endGame);
  ...
  `;
        ...
        document.getElementById('voltarSaqueForm').onclick = (typeof onVoltar === 'function' ? onVoltar : endGame);
        ...
        document.getElementById('formSaquePix').onsubmit = async (e) => {
            ...
            if (err.includes('rollover') || err.includes('apostar') || err.includes('Saldo insuficiente')) {
                showSaqueError(err);
                return;
            }
            showSaqueError(err);
    ...
    function showHistoricoModal() {
        ...
        function renderDeposits(list) {
            if (!list || list.length === 0) return '<p class="modalHistoricoVazio">Nenhum depósito encontrado.</p>';
            var html = '<ul class="modalHistoricoLista">';
            list.forEach(function(r) {
                var valor = fmtMoney(r.amount);
                var bonus = r.bonus_amount != null && Number(r.bonus_amount) > 0 ? ' (+' + fmtMoney(r.bonus_amount) + ' bônus)' : '';
                var status = (r.status || '').toLowerCase();
                var label = statusDepositoLabel(r.status);
                html += '<li class="modalHistoricoItem"><span class="modalHistoricoItemValor">R$ ' + valor + bonus + '</span><span class="modalHistoricoItemStatus modalHistoricoStatus-' + status + '">' + label + '</span><span class="modalHistoricoItemData">' + fmtDate(r.created_at) + '</span></li>';
            });
            html += '</ul>';
            return html;
        }
        ...
        function renderWithdrawals(list) {
            if (!list || list.length === 0) return '<p class="modalHistoricoVazio">Nenhum saque encontrado.</p>';
            var html = '<ul class="modalHistoricoLista">';
            list.forEach(function(r) {
                var valor = fmtMoney(r.amount);
                var status = (r.status || '').toLowerCase();
                var label = statusSaqueLabel(r.status);
                html += '<li class="modalHistoricoItem"><span class="modalHistoricoItemValor">R$ ' + valor + '</span><span class="modalHistoricoItemStatus modalHistoricoStatus-' + status + '">' + label + '</span><span class="modalHistoricoItemData">' + fmtDate(r.created_at) + '</span></li>';
            });
            html += '</ul>';
            return html;
        }
        ...
        function renderGames(list) {
            if (!list || list.length === 0) return '<p class="modalHistoricoVazio">Nenhuma jogada encontrada.</p>';
            var html = '<ul class="modalHistoricoLista">';
            list.forEach(function(r) {
                var aposta = fmtMoney(r.bet_amount);
                var ganho = fmtMoney(r.win_amount);
                html += '<li class="modalHistoricoItem"><span class="modalHistoricoItemValor">Aposta R$ ' + aposta + '</span><span class="modalHistoricoItemValor">Ganho R$ ' + ganho + '</span><span class="modalHistoricoItemData">' + fmtDate(r.created_at) + '</span></li>';
            });
            html += '</ul>';
            return html;
        }
        ...
        function loadHistorico() {
            ...
            var promise;
            if (tabAtual === 'deposits') promise = typeof getHistoryDeposits === 'function' ? getHistoryDeposits(pageAtual) : Promise.resolve({
                success: false
            });
            else if (tabAtual === 'withdrawals') promise = typeof getHistoryWithdrawals === 'function' ? getHistoryWithdrawals(pageAtual) : Promise.resolve({
                success: false
            });
            else promise = typeof getHistoryGames === 'function' ? getHistoryGames(pageAtual) : Promise.resolve({
                success: false
            });
            ...
            promise.then(function(res) {
                if (!container) return;
                if (!res.success) {
                    container.innerHTML = '<p class="modalHistoricoVazio">' + (res.error || 'Erro ao carregar. Tente novamente.') + '</p>';
                    return;
                }
                var data = res.data || {};
                ...
                if (pagEl && pagination.total_pages > 1) {
                    ...
                    pagEl.innerHTML = '<span class="modalHistoricoPagInfo">Página ' + pagination.current_page + ' de ' + pagination.total_pages + '</span>' +
                        '<button type="button" class="modalHistoricoPagBtn" id="historicoPagPrev"' + (prevDisabled ? ' disabled' : '') + '>Anterior</button>' +
                        '<button type="button" class="modalHistoricoPagBtn" id="historicoPagNext"' + (nextDisabled ? ' disabled' : '') + '>Próxima</button>';
                    var prevBtn = document.getElementById('historicoPagPrev');
                    var nextBtn = document.getElementById('historicoPagNext');
                    if (prevBtn && !prevDisabled) prevBtn.onclick = function() {
                        pageAtual--;
                        loadHistorico();
                    };
                    if (nextBtn && !nextDisabled) nextBtn.onclick = function() {
                        pageAtual++;
                        loadHistorico();
                    };
                }
            }).catch(function() {
                if (container) container.innerHTML = '<p class="modalHistoricoVazio">Erro ao carregar. Tente novamente.</p>';
            });
        }
  ...
  `;
        ...
        if (btnVoltar) btnVoltar.onclick = function() {
            modal.style.display = 'none';
            location.reload();
        };
  ...
  `;
    ...
    function applyAffiliateStatsToDom(data) {
        ...
        set('afiliadosComissoes', fmtMoney(data.generated_commission != null ? data.generated_commission : data.total_commission));
        ...
        set('afiliadosAguardandoDeposito', fmtNum(data.status_counts && data.status_counts.awaiting_deposit));
        ...
        set('afiliadosModoCpa', data.commission_mode === 'all_deposits' ? ('todos / pular ' + fmtNum(data.skip_cpa || 0)) : 'primeiro');
        var saldoMsg = document.getElementById('afiliadosSemSaldoMsg');
        if (saldoMsg) saldoMsg.style.display = (data.available_commission == null || Number(data.available_commission) <= 0) ? 'block' : 'none';
        ...
        if (leadList) {
            ...
            if (!records.length) {
                leadList.innerHTML = '<div style="padding:10px 0;color:#bbf7d0;font-size:12px;">Nenhum indicado encontrado.</div>';
            } else {
                leadList.innerHTML = records.map(function(record) {
                    var reason = record.status_reason ? '<div style="font-size:10px;color:#d1d5db;margin-top:4px;">' + record.status_reason + '</div>' : '';
                    return ''
                        + '<div style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,.12);text-align:left;">'
                        + '<div style="display:flex;justify-content:space-between;gap:10px;">'
                        + '<div>'
                        + '<div style="font-size:13px;font-weight:700;color:#fff;">' + (record.nome || 'Indicado') + '</div>'
                        + '<div style="font-size:10px;color:#bbf7d0;">' + (record.telefone || '') + '</div>'
                        + '</div>'
                        + '<div style="font-size:10px;font-weight:700;color:#fef08a;text-transform:uppercase;">' + (record.status_label || '-') + '</div>'
                        + '</div>'
                        + '<div style="margin-top:6px;font-size:11px;color:#e5e7eb;">Confirmados: ' + fmtNum(record.confirmed_deposit_count || 0) + ' | Comissao: ' + fmtMoney(record.commission_total || 0) + '</div>'
                        + reason
                        + '</div>';
                }).join('');
            }
  ...
  `;
        ...
        if (walletBox) {
            ...
            blockedMsg.textContent = 'Esta area libera comissoes apenas para contas aprovadas como afiliado.';
        ...
        function loadStats(p) {
            ...
            getAffiliateStats(p).then(function(res) {
        ...
        if (btnSolicitarSaque && formSaqueWrap) {
            btnSolicitarSaque.onclick = function() {
                var saldo = typeof window.__pandaAfiliadosSaldoDisponivel === 'number' ? window.__pandaAfiliadosSaldoDisponivel : 0;
                if (saldo <= 0) return;
        ...
        if (btnConfirmarSaque) {
            btnConfirmarSaque.onclick = function() {
                ...
                if (valor > saldo) {
                    alert('Valor maior que o saldo disponível (R$ ' + saldo.toFixed(2).replace('.', ',') + ').');
                    return;
                }
                if (!pixKey) {
                ...
                createAffiliateWithdraw({
                    ...
                    if (res.success) {
                        ...
                        var msg = (res.data && res.data.message) ? res.data.message : 'Saque solicitado com sucesso! Aguarde aprovação do administrador.';
  ...
  `;
        ...
        document.getElementById('backToGameOver').onclick = (typeof onVoltar === 'function' ? onVoltar : endGame);
        ...
        function atualizarBonusDisplay() {
            ...
            if (op && op.bonus > 0) {
                bonusMsg.textContent = '🎁 Você ganha R$ ' + op.bonus.toFixed(2).replace('.', ',') + ' de bônus!';
    ...
    function fillWinnerFeedRecarga() {
        ...
        if (!track) return;
        var nomes = ['mar***osa', 'cam***ira', 'lea***', 'pau***', 'rod***ues', 'ana***ira', 'ped***elo', 'jul***ima', 'fer***des', 'luc***tos', 'raf***ira', 'bru***ves', 'gui***rme', 'pat***cia', 'dan***tos', 'tha***ara', 'leo***dro', 'bia***a', 'gab***la', 'ric***do', 'car***nos', 'vit***ria', 'and***ra', 'jos***va'];

        function randomItem() {
            var valor = Math.round((Math.random() * (500 - 20) + 20) * 100) / 100;
            var nome = nomes[Math.floor(Math.random() * nomes.length)];
            return '<span class="winner-feed-item"><span class="valor shimmer-text">R$ ' + valor.toFixed(2).replace('.', ',') + '</span> <span class="user">' + nome + '</span> <span class="ganhou">SACOU</span></span>';
        }
        var html = '';
    ...
    async function processarValorPersonalizado() {
        const valorInput = document.getElementById('valorPersonalizado');
        const valorStr = (valorInput && valorInput.value) ? String(valorInput.value).replace(',', '.') : '';
    ...
    function mostrarQRCode(pixData) {
        ...
        const qrCodeUrl = pixData.qrCodeImage || `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(pixData.qr_code_text)}`;
        const valorFormatado = (pixData.amount || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
        ...
        document.getElementById('copiarCodigoPix').onclick = () => {
            ...
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(link).then(() => {
                    ...
                    setTimeout(() => { if (btn) btn.textContent = '📋 Copiar Código PIX'; }, 2000);
                }).catch(() => {
                    // Fallback
                    if (textarea) {
                        textarea.select();
            ...
            if (isFreeGame) {
                ...
                if (typeof getBalance === 'function') {
                    ...
                    if (res.success && res.data != null) {
                        ...
                        if (saldoEl) saldoEl.innerText = 'Saldo: R$ ' + Number(res.data.balance).toFixed(2).replace('.', ',');
                ...
                if (typeof getPandaConfig === 'function') {
                    ...
                    if (cfg.success && cfg.data) {
                        ...
                        applyPandaConfigData(cfg.data);
                         ...
                         if (typeof cfg.data.withdraw_multiplier === 'number') window.pandaWithdrawMultiplier = cfg.data.withdraw_multiplier;
...
const inp = document.getElementById('valorAposta');
                        if (inp) {
                            ...
                            if (typeof cfg.data.min_bet === 'number' && !inp.value) inp.placeholder = cfg.data.min_bet.toFixed(2).replace('.', ',');
                        }
                    }
                }
                atualizarModoNomeNoFront();
    ...
    (function initModalAuth() {
        ...
        if (menuTopoDeposito) menuTopoDeposito.onclick = function() {
            menuTopoDropdown.style.display = 'none';
            showRechargeModal(() => location.reload());
        };
        if (menuTopoSaque) menuTopoSaque.onclick = async function() {
            menuTopoDropdown.style.display = 'none';
            const res = await getBalance();
            if (res.success && res.data != null) {
        ...
        if (menuTopoSair) menuTopoSair.onclick = async function() {
            menuTopoDropdown.style.display = 'none';
        ...
        if (menuTopoAfiliados) menuTopoAfiliados.onclick = function() {
            menuTopoDropdown.style.display = 'none';
            showAfiliadosModal();
        };
        var menuTopoHistorico = document.getElementById('menuTopoHistorico');
        if (menuTopoHistorico) menuTopoHistorico.onclick = function() {
            menuTopoDropdown.style.display = 'none';
            showHistoricoModal();
        };
        ...
        try {
            var refParams = new URLSearchParams(typeof window !== 'undefined' ? (window.location.search || '') : '');
        ...
        function showInicio(balanceData) {
            if (typeof window.__pandaOnLoginSuccess === 'function') {
                window.__pandaOnLoginSuccess(balanceData && balanceData.balance != null ? balanceData.balance : null);
                return;
            }
        ...
        if (telRegister) telRegister.addEventListener('input', function() {
            formatPhoneInput(this);
        });
        ...
        if (formLogin) {
            formLogin.addEventListener('submit', async (e) => {
                ...
                const tel = (document.getElementById('authTelefoneLogin') || {}).value.trim().replace(/\D/g, '');
                ...
                showError('');
                try {
                    const res = await login(tel.length >= 10 ? tel : document.getElementById('authTelefoneLogin').value.trim(), senha);
                    if (!res.success) {
        ...
        if (formRegister) {
            formRegister.addEventListener('submit', async (e) => {
                ...
                const tel = (document.getElementById('authTelefoneRegister') || {}).value.trim().replace(/\D/g, '');
                const cpf = (document.getElementById('authCpfRegister') || {}).value.trim().replace(/\D/g, '');
