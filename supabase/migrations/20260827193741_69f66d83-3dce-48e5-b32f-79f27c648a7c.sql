...
INSERT INTO public.app_settings (key, value, is_public) VALUES
  ('gateway', '{"deposito_min":10,"deposito_max":5000,"saque_min":30,"saque_max":5000,"taxa_saque":0}'::jsonb, true),
  ('afiliado', '{"comissao_cpa":10,"comissao_cpa_nivel2":2,"comissao_revshare":10,"deposito_minimo_cpa":20}'::jsonb, false)
ON CONFLICT (key) DO NOTHING;

INSERT INTO public.game_settings (slug, value) VALUES
  ('rtp', 90),
  ('aposta_min', 1),
  ('aposta_max', 500)
...
BEGIN
  ...
  SELECT COALESCE((SELECT value FROM public.game_settings WHERE slug = 'aposta_min' AND user_id IS NULL), 1),
         COALESCE((SELECT value FROM public.game_settings WHERE slug = 'aposta_max' AND user_id IS NULL), 500)
    INTO _min, _max;
  ...
  UPDATE public.profiles
     SET saldo_bonus = COALESCE(saldo_bonus, 0) - _from_bonus,
         saldo = COALESCE(saldo, 0) - _from_saldo,
         total_apostado = COALESCE(total_apostado, 0) + _aposta
   WHERE id = _uid;
...
BEGIN
  ...
  SELECT * INTO _s FROM public.game_sessions
   WHERE id = _session_id AND user_id = _uid FOR UPDATE;
  IF NOT FOUND THEN
  ...
  IF NOT _s.is_demo AND _win > 0 THEN
    UPDATE public.profiles SET saldo = COALESCE(saldo, 0) + _win WHERE id = _uid;
  END IF;
...
REVOKE ALL ON FUNCTION public.game_place_bet(numeric, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.game_settle_bet(uuid, numeric, text, jsonb) FROM PUBLIC;
